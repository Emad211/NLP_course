...
DATA_DIR = BASE_DIR / "processed_data" / "sentiment"

TRAIN_CSV = DATA_DIR / "train.csv"
DEV_CSV = DATA_DIR / "dev.csv"
TEST_CSV = DATA_DIR / "test.csv"

MODEL_OUT = BASE_DIR / "models" / "sentiment_mlp.joblib"
...
def load_split(path: Path):
    ...
    # ستون متن: ترجیحاً text_label
    if "text_label" not in df.columns:
        raise RuntimeError(f"Missing text_label in {path.name}. Columns={list(df.columns)}")
    # ستون لیبل: label_human_int
    if "label_human_int" not in df.columns:
        raise RuntimeError(f"Missing label_human_int in {path.name}. Columns={list(df.columns)}")
...
def main():
    ...
    vec = TfidfVectorizer(
        lowercase=True,
        ngram_range=(1, 2),
        min_df=1,          # با دیتاست کوچک بهتره 1 باشه
        max_df=0.98,
        sublinear_tf=True,
        max_features=60000
    )
    ...
    lines = []
    lines.append("SENTIMENT MLP REPORT (TF-IDF + MLPClassifier)")
    ...
    lines.append("")
    lines.append(eval_split("DEV", y_dev, dev_pred))
    lines.append(eval_split("TEST", y_te, test_pred))
